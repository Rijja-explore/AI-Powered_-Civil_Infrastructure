#!/usr/bin/env python3
"""
Model Training for Vegetation Detection using YOLOv8
Trains on vegetation_preprocess dataset and saves metrics
"""
...
RUNS_DIR = Path("runs/detect/vegetation")
METRICS_OUTPUT = Path("metrics_vegetation.json")
...
def create_dataset_yaml():
    ...
    dataset_config = {
        ...
        'names': ['Moss', 'Algae', 'Lichen', 'Plants']  # class names
    }
    
    yaml_path = DATASET_PATH.parent / 'vegetation_dataset.yaml'
    with open(yaml_path, 'w') as f:
        yaml.dump(dataset_config, f)
    
    return yaml_path

def train_vegetation_model():
    """Train YOLOv8 model for vegetation detection"""
    print("🚀 Starting Vegetation Detection Model Training...")
    ...
    metrics_dict = {
        ...
        'class_metrics': {
            'Moss': {'precision': 0.0, 'recall': 0.0, 'mAP50': 0.0},
            'Algae': {'precision': 0.0, 'recall': 0.0, 'mAP50': 0.0},
            'Lichen': {'precision': 0.0, 'recall': 0.0, 'mAP50': 0.0},
            'Plants': {'precision': 0.0, 'recall': 0.0, 'mAP50': 0.0}
        },
        'dataset_info': {
...
def main():
    ...
    train_vegetation_model()
    print("✨ Vegetation detection model training complete!")
